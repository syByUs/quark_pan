...
class QuarkPanFileManager:
    def __init__(self, headless: bool = False, slow_mo: int = 0) -> None:
        ...
        print(self.cookies, 'quark cookies')
    ...
    # 获取分享文件名
    async def get_share_file_name(self, input_line: str):
        ...
        if not stoken:
            return None
        _, data_list = await self.get_detail(pwd_id, stoken)
        print("get_share_file_name:", data_list)
        """
        [{'fid': 'd7213d336bce4df397a8d6b9c67b71b8', 'file_name': '《漫画数学王》
[全三册]', 'file_type': 0, 'dir': True, 'pdir_fid': '9909d15a2f9d4730a932733ed0ab5ed7', 'include_items': 6, 'share_fid_token': '4223e79d5c2991b8305922aae022a600', 'status': 1}]
        """
        return data_list, pwd_id, stoken
    ...
    async def run(self, input_line: str, folder_id: Union[str, None] = None, download: bool = False) -> None:
        ...
        print("run get_detail:", data_list)
        files_count = 0
        folders_count = 0
        files_list: List[str] = []
    ...
    async def load_folder_id(self, renew=False) -> Union[tuple, None]:
        ...
        if renew:
            ...
            if pdir_id == '0':
                ...
                file_list_data = await self.get_sorted_file_list() # 获取文件夹列表
            ...
            if traverse_depth == 0:
                try:
                    ...
                    with open(save_share_path, 'a', encoding='utf-8') as f:
                        ...
                        f.write(content + '\n')
                        custom_print(f'分享 【{title}】 成功，链接地址【{share_url}】')
                    return share_url
...
def exe_share_run():
    ...
    while True:
        ...
        print("to_dir_id：", to_dir_id, "to_dir_name：", to_dir_name)
        ...
        if input_text and input_text.strip() in [str(i) for i in range(1, 7)]:
            ...
            # 2分享
                ...
                if share_option and share_option == '1':
                    print("url_encrypt:", url_encrypt)
                    print("_expired_type:", _expired_type)
                    print("passcode:", passcode)
                    print("_traverse_depth:", _traverse_depth)
